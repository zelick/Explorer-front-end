...
export class PlanYourTripComponent implements OnInit, AfterViewInit{
  ...
  constructor(private service: TourAuthoringService,private mapService: MapService, private tourAuthoringService: TourAuthoringService, private router: Router){

  }
  updateTours(){
    this.service.getToursWithPublicCheckpoints(this.selectedCheckpoints).subscribe({
      next: (result)=>{
        this.tours = result;
      }
    });
